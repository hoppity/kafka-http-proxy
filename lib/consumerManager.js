...
var get = function (group, id, callback) {
        if (arguments.length < 1) {
            callback(new Error('Group and Instance ID or ID required.'));
        }
    ...
    add = function (consumer, callback) {
        ...
        if (!!consumers[consumer.id]) {
            callback(new Error('Consumer with ID ' + consumer.id + ' already exists.'));
        }
    ...
    createConsumerInstance = function (consumer, topic) {
        ...
        consumer.instance.on('rebalancing', function(event){
            logger.warn('lib/consumerManager : Consumer rebalancing');
            consumer.messages = [];
        });
